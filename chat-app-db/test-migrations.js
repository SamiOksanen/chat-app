...
async function testDatabaseSchema() {
    ...
    `;
        ...
        const expectedMessagesColumns = [
            'has_markdown',
            'mentioned_user_ids',
            'updated_at',
        ];
        const foundMessagesColumns = messagesResult.rows.map(
            (row) => row.column_name
        );
        ...
        for (const expectedCol of expectedMessagesColumns) {
            if (!foundMessagesColumns.includes(expectedCol)) {
                throw new Error(
                    `Expected column '${expectedCol}' not found in messages table`
                );
            }
        }
    ...
    `;
        ...
        const foundUsersColumns = usersResult.rows.map(
            (row) => row.column_name
        );
        ...
        for (const expectedCol of expectedUsersColumns) {
            if (!foundUsersColumns.includes(expectedCol)) {
                throw new Error(
                    `Expected column '${expectedCol}' not found in users table`
                );
            }
        }
    ...
    `;
        ...
        for (const expectedIndex of expectedIndexes) {
            if (!foundIndexes.includes(expectedIndex)) {
                console.warn(
                    `Warning: Expected index '${expectedIndex}' not found`
                );
            }
        }
    ...
    `;
        ...
        // Clean up test data
        await client.query('DELETE FROM messages WHERE messageid = $1', [
            message.messageid,
        ]);
        await client.query(
            'DELETE FROM conversations WHERE conversationid = $1',
            [conversationId]
        );
