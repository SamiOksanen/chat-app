...
async function getAppliedSeeds(client) {
  try {
    const result = await client.query('SELECT version FROM seed_migrations ORDER BY version');
    return new Set(result.rows.map(row => row.version));
  } catch (error) {
    console.log('Seed migrations table does not exist yet, will be created');
    return new Set();
  }
}
