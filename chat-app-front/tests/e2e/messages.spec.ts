...
test.describe('Messages', () => {
  ...
  test('should send a new message', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="message-item"]', {
      hasText: testMessage
  ...
  test('should not send empty message', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="message-item"]')).toHaveCount(initialMessageCount);
  });

  test('should scroll to bottom when new message arrives', async ({ page }) => {
    // Scroll to top to ensure we're not at bottom
    await page.locator('[data-testid="messages-container"]').evaluate(el => el.scrollTop = 0);
  ...
  test('should display messages in chronological order', async ({ page }) => {
    ...
    if (messageCount > 1) {
      ...
      const firstTimestamp = await messages.first().locator('[data-testid="message-timestamp"]').textContent();
      const lastTimestamp = await messages.last().locator('[data-testid="message-timestamp"]').textContent();
  ...
  test('should handle long messages properly', async ({ page }) => {
    const longMessage = 'This is a very long message that should wrap properly and not break the UI layout. '.repeat(10);
    ...
    const containerWidth = await page.locator('[data-testid="messages-container"]').evaluate(el => el.clientWidth);
