...
test.describe('Conversations', () => {
  ...
  test('should display group conversations with group names', async ({ page }) => {
    // Look for group conversation (Test Group Chat from seed data)
    const groupConversation = page.locator('[data-testid="conversation-item"]', {
      hasText: 'Test Group Chat'
    });
    await expect(groupConversation).toBeVisible();

    // Should show group indicator
    await expect(groupConversation.locator('[data-testid="group-indicator"]')).toBeVisible();
  });

  test('should display direct message conversations with usernames', async ({ page }) => {
    // Look for direct message conversations
    const dmConversations = page.locator('[data-testid="conversation-item"][data-conversation-type="direct"]');
